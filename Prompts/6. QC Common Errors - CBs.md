...
You receive a **task directory path** containing all deliverables. All files are read directly from the directory.
...
├── stdout_fail.log                  # Raw test output before patch
├── stderr_fail.log                  # Raw test stderr before patch
├── stdout_success.log               # Raw test output after patch
├── stderr_success.log               # Raw test stderr after patch
├── container_output/                # Contains results from review command execution
│   ├── test_results_fail.json       # Must match root-level test_results_fail.json
│   ├── test_results_success.json    # Must match root-level test_results_success.json
│   ├── workflow.log
│   └── ...
└── ...                              # Other files (dockerfiles, build.sh, parsing.py, etc.)
```

**Note on main_language**: Always infer from file extensions in the code_patch (e.g., `.rb` = Ruby, `.py` = Python, `.go` = Go, `.ts`/`.js` = TypeScript/JavaScript, `.cs` = C#, `.cpp`/`.h` = C++, `.java` = Java).
...
**The PS + Requirements + Tests alignment (Section 10) is the #1 source of QC flags.** Pay extra attention to it.

## Checks Summary

| # | Error Code | Check | Inputs Used |
|---|---|---|---|
| 1 | `appConfigInTestPatch` | App config files must NOT be in test_patch | test_patch |
| 2 | `testsInCodePatch` | Test files must NOT be in code_patch | code_patch |
| 3 | `dirtyTestCommand` | Test command must only run tests — no service setup, installs, or infra commands | test_command |
| 4 | `testResultsMismatch` | Same tests must appear in both fail and success JSON files (same names, same count) | test_results_fail, test_results_success |
| 5 | `regressionInFailResults` | No tests with PASSED status in test_results_fail.json | test_results_fail |
| 6 | `containerOutputMismatch` | Root-level JSON results must match container_output/ JSON results; container_output/ must exist | test_results_fail, test_results_success, container_output/ |
| 7 | `staticTests` | Tests must exercise actual behavior, not just check existence (automatic score 2) | test_patch |
| 8 | `rubyMethodNaming` | Ruby interfaces must use `.` not `#` (Ruby only) | interfaces, code_patch |
| 9 | `rubyHashInResults` | No `#` in test result names (Ruby only) | test_results_fail, test_results_success |
| 10a | `overspecifiedTests` | Tests must not assert values the agent can't deduce from PS + Reqs + Interfaces | test_patch, problem_statement, requirements, interfaces |
| 10b | `underspecifiedDocumentation` | PS + Reqs must contain all info needed for tests to pass | test_patch, problem_statement, requirements, interfaces |
| 10c | `documentationTestMismatch` | PS/Reqs must not contradict what tests expect | test_patch, problem_statement, requirements, interfaces |
| 11 | `reviewCommandsNotRun` | Signs that review commands were not executed | container_output/, workflow.log |

**Every error is a QC flag — there are no exceptions. Any error found = FAIL.**
...
### 6. Container Output vs Task Directory Consistency (containerOutputMismatch)

**RULE**: The JSON test result files at the task directory root level must be semantically identical to their counterparts inside the `container_output/` directory. The order of tests may differ, but the content (test names and statuses) must match exactly.

Both locations must contain the same files:
- `test_results_fail.json` (root) must match `container_output/test_results_fail.json`
- `test_results_success.json` (root) must match `container_output/test_results_success.json`

**JSON format reference** — both files use this structure:
```json
{
  "tests": [
    {"name": "path/to/test_file.py | TestClass test_method_name", "status": "PASSED"},
    {"name": "path/to/test_file.py | TestClass test_other_method", "status": "FAILED"}
  ]
}
```

The `name` field typically contains the test file path followed by `|` and the test description/identifier. The `status` field is either `"PASSED"` or `"FAILED"`.

**Flag if:**
- A JSON file exists at the root but is missing from `container_output/` (or vice versa)
- The `tests` arrays contain different test names (ignoring order)
- The status of a test differs between the root file and the container_output file (e.g., `FAILED` in root but `PASSED` in container_output)
- The number of tests differs between root and container_output versions of the same file

**Common cause**: The contributor manually edited the root-level JSON files after running the tests, or copied an older version of the results. The container_output files are generated by the review commands and represent the actual results from the container execution.

**Also flag if:**
- The `container_output/` directory does not exist at all — this means the review commands were likely never run.

### 7. Static Tests (staticTests)
...
### 10. PS + Requirements + Tests Alignment (PRIMARY CHECK)
...
| Sub-check | Error Code | What it catches |
|---|---|---|
| 10a | `overspecifiedTests` | Tests assert values the agent can't know |
| 10b | `underspecifiedDocumentation` | PS/Reqs missing info that tests depend on |
| 10c | `documentationTestMismatch` | PS/Reqs say X, tests expect Y |
...
**Every error detected by this evaluation is a QC flag — there are no exceptions.** All checks in this prompt represent issues that QC has flagged in real submissions. If any error is found, the submission FAILS.

The following errors are additionally marked as **automatic score of 2** (critical — task is fundamentally broken):
...
### PASS
Use when:
- No errors are found across ALL checks
- The submission follows all process guidelines

### FAIL
Use when:
- ANY error is found — every error in this evaluation is a QC flag, no exceptions
...
```json
{
  ...
  "errors": [
    {
      "errorCode": "appConfigInTestPatch | testsInCodePatch | dirtyTestCommand | testResultsMismatch | regressionInFailResults | containerOutputMismatch | staticTests | rubyMethodNaming | rubyHashInResults | overspecifiedTests | underspecifiedDocumentation | documentationTestMismatch | reviewCommandsNotRun",
...
- **overall_label**: Exactly one of: "PASS" or "FAIL"
...
{
  "tests": [
    {"name": "tests/test_user_api.py | TestUserAPI test_create_user_returns_201", "status": "FAILED"},
    {"name": "tests/test_user_api.py | TestUserAPI test_create_user_validates_email", "status": "FAILED"},
    {"name": "tests/test_user_api.py | TestUserAPI test_create_user_rejects_duplicate", "status": "FAILED"}
  ]
}
</example_test_results_fail>
...
{
  "tests": [
    {"name": "tests/test_user_api.py | TestUserAPI test_create_user_returns_201", "status": "PASSED"},
    {"name": "tests/test_user_api.py | TestUserAPI test_create_user_validates_email", "status": "PASSED"},
    {"name": "tests/test_user_api.py | TestUserAPI test_create_user_rejects_duplicate", "status": "PASSED"}
  ]
}
</example_test_results_success>
...
{
  "tests": [
    {"name": "spec/requests/api_spec.rb | API::Balances GET /balances returns error", "status": "FAILED"}
  ]
}
</example_test_results_fail>
...
{
  "tests": [
    {"name": "spec/requests/api_spec.rb | API::Balances GET /balances returns balances list", "status": "PASSED"},
    {"name": "spec/requests/api_spec.rb | API::Balances GET /balances returns error for invalid params", "status": "PASSED"},
    {"name": "spec/requests/api_spec.rb | API::Balances GET /balances/:id returns balance details", "status": "PASSED"},
    {"name": "spec/requests/api_spec.rb | API::Balances POST /balances creates a balance", "status": "PASSED"},
    {"name": "spec/requests/api_spec.rb | API::Balances DELETE /balances/:id deletes a balance", "status": "PASSED"}
  ]
}
</example_test_results_success>
...
```json
{
  ...
  "summary": "The submission has two QC flags: a polluted test command with infrastructure setup and a mismatch in test results (1 fail vs 5 success). Both must be resolved before submission.",
  "testResultsAnalysis": {
    "failTestCount": 1,
    "successTestCount": 5,
    "testsMatch": false,
...
### Example 4: Regression Tests in Fail Results — FAIL

<example_test_results_fail>
{
  "tests": [
    {"name": "tests/test_feature.py | TestFeature test_new_feature_works", "status": "FAILED"},
    {"name": "tests/test_feature.py | TestFeature test_new_feature_edge_case", "status": "FAILED"},
    {"name": "tests/test_feature.py | TestFeature test_existing_functionality_still_works", "status": "PASSED"}
  ]
}
</example_test_results_fail>
...
{
  "tests": [
    {"name": "tests/test_feature.py | TestFeature test_new_feature_works", "status": "PASSED"},
    {"name": "tests/test_feature.py | TestFeature test_new_feature_edge_case", "status": "PASSED"},
    {"name": "tests/test_feature.py | TestFeature test_existing_functionality_still_works", "status": "PASSED"}
  ]
}
</example_test_results_success>
...
### Example 5: Ruby Method Naming + Hash in Results — FAIL
...
{
  "tests": [
    {"name": "spec/models/user_spec.rb | User#validate_email returns false for invalid email", "status": "FAILED"},
    {"name": "spec/models/user_spec.rb | User#validate_email returns true for valid email", "status": "FAILED"}
  ]
}
</example_test_results_fail>
...
```json
{
  ...
  "summary": "Two Ruby-specific formatting issues found: hash notation (#) used instead of dot notation (.) in both the interfaces section and test results files. Both are QC flags that must be corrected.",
  "testResultsAnalysis": {
    "failTestCount": 2,
    "successTestCount": 2,
    "testsMatch": true,
...
```json
{
  ...
  "summary": "Two file organization QC flags: app-level configs (database.yml, .env.test) are in the test_patch instead of code_patch, and a test file appears in the code_patch. Both violate the patch separation rules and must be fixed.",
  "testResultsAnalysis": {
    "failTestCount": 1,
    "successTestCount": 1,
    "testsMatch": true,
...
4. `test_results_fail.json` — test results before applying code patch (root level)
5. `test_results_success.json` — test results after applying code patch (root level)
6. `container_output/test_results_fail.json` — test results from container execution
7. `container_output/test_results_success.json` — test results from container execution
8. `0. Problem statement.md` — the problem statement
9. `1. Interfaces.md` — the public interfaces documentation
10. `2. Requirements.md` — the requirements documentation

Infer the **main language** from the file extensions in `code_patch.patch`.

---

Evaluate all inputs against the criteria above and produce the output.
