...
The Problem Statement must provide sufficient detail for an experienced software engineer to implement a solution that passes all unit tests, using only the Problem Statement and Requirements (without access to the code patch).

**Specificity Levels:**

| Level | Description | Result |
|-------|-------------|--------|
| **1-2** | Nearly impossible to understand what needs to be implemented. Requirements do not define what is needed to pass tests. Critical information is completely missing. | INCORRECT |
| **3** | Ambiguous with important details missing. General description exists but misses critical information needed to pass at least one unit test. Some behaviors could be inferred but meaningful gaps remain. | INCORRECT |
| **4** | Small gaps to fill, but correct solution can be reasonably inferred from the repository. A developer could produce a valid implementation if Requirements are interpreted correctly. | Acceptable |
| **5** | Clear and fully specified. Requirements explicitly describe expected behavior. Implementing exactly what is stated should result in all unit tests passing. | Perfect |

**Important Notes on Specificity:**
- The level of specificity must match what the tests actually require
- If the code mentions features not validated by tests, the Problem Statement should only describe what is tested
- Focus on under-specificity (critical) rather than over-specificity (acceptable)
- The Problem Statement should describe behaviors centered on what the tests validate, not what the code patch contains beyond testing

**Examples of Under-Specificity (Levels 1-3):**
- "Something is wrong with the API" (Level 1)
- "Fix the calculation" without specifying inputs/outputs (Level 2)
- "Handle errors properly" without specifying which errors or how (Level 3)
- Describing main behavior but tests require specific null handling not mentioned (Level 3)

**Examples of Adequate Specificity (Levels 4-5):**
- Main behavior clear, edge cases inferable from codebase (Level 4)
- PS + Requirements fully define all inputs, outputs, edge cases, and error handling (Level 5)

### 7. Language Agnostic Description
...
- **Too vague or underspecified (Specificity Level 1-3) - cannot determine what needs to be implemented based on PS + Requirements alone**
...
### Example 11: Underspecified Problem Statement (Specificity Level 2)
...
<example_problem_statement>
```
## Title: Phone validation needed

### Description
The system needs phone number validation.
```
</example_problem_statement>
...
```json
{
  "correctness_label": "INCORRECT",
  "issues": [
    "Severely underspecified (Specificity Level 2) - does not describe validation rules",
    "Missing critical information: minimum length requirement, digit-only requirement, empty value handling",
    "An implementer would have to guess the validation logic"
  ],
  "explanation": "This Problem Statement is at Specificity Level 2 - it gives a minimal idea of what is expected but misses critical information needed to pass unit tests. The description should specify that phone numbers must be non-empty, at least 10 digits, and contain only numeric characters. Without these details, an implementer cannot create a solution that passes the tests."
}
```
</example_output>

### Example 12: Adequate Specificity (Level 4-5)
...
<example_problem_statement>
```
## Title: Add phone number validation

### Description
The system currently accepts any input as a phone number, leading to invalid data being stored. Phone number validation is needed to ensure data quality.

The validation should enforce the following rules:
- Phone numbers cannot be empty
- Phone numbers must be at least 10 digits long
- Phone numbers must contain only numeric digits (0-9)

When validation fails, the system should provide clear error messages indicating which rule was violated.
```
</example_problem_statement>
...
```json
{
  "correctness_label": "CORRECT",
  "issues": [],
  "explanation": "This Problem Statement is at Specificity Level 5 - it clearly describes all validation rules that the tests will verify. An implementer can create a solution that passes all tests based solely on this description. It describes WHAT validation is needed without prescribing HOW to implement it (no specific functions, algorithms, or code patterns mentioned)."
}
```
</example_output>

### Example 13: Specificity Level 3 - Missing Critical Detail
...
### Description
The calculator needs a divide function to perform division operations on two numbers. The function should handle edge cases appropriately.
```
</example_problem_statement>
...
```json
{
  "correctness_label": "INCORRECT",
  "issues": [
    "Specificity Level 3 - mentions 'edge cases' but doesn't specify what they are or how to handle them",
    "Critical missing detail: does not specify that division by zero should return infinity",
    "An implementer might raise an exception, return None, or return 0 for division by zero"
  ],
  "explanation": "This Problem Statement is at Specificity Level 3 - it describes the main behavior (division) but misses the critical detail about handling division by zero. The tests will fail if an implementer chooses to raise an exception instead of returning infinity. The PS should explicitly state: 'When dividing by zero, the function should return infinity rather than raising an error.'"
}
```
</example_output>
...
Give me a file with the output of this, named {Problem_Statement.json}