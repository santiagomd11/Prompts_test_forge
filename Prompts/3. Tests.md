...
Tests MUST verify behavior through the PUBLIC interface ONLY. Any direct access to private, protected, internal, or conventionally-private members is INCORRECT.

**STRICT RULE: If a method, function, attribute, or class member is NOT part of the documented public API, tests MUST NOT access it directly.**

**Mark as INCORRECT (privateAccessTests) if tests do ANY of the following:**
- Call ANY method/function with underscore prefix (`_method`, `__method`)
- Access ANY attribute/property with underscore prefix (`_attr`, `__attr`)
...
- Access dunder attributes for testing purposes (`__dict__`, `__class__`, `__ray_actor_class__`, etc.)
- Use language-specific tricks to bypass visibility (`.send()`, `setAccessible()`, `as any`, etc.)
...
|---------|---------|-------------------|
| Single underscore method | `obj._method()` | Private by convention |
| Single underscore attribute | `obj._cache` | Private by convention |
| Double underscore method | `obj.__method()` | Name-mangled private |
| Name mangling access | `obj._Class__method()` | Bypassing private |
| Dunder internal access | `Class.__ray_actor_class__` | Internal implementation |
| Dict access for private | `obj.__dict__["_key"]` | Bypassing encapsulation |
| Lifecycle hooks | `obj._pre_loop_hook()` | Internal lifecycle |
| Internal methods | `obj._execute()`, `obj._run()` | Internal implementation |

```python
# ❌ BAD - ALL OF THESE ARE INCORRECT:

# Calling ANY method starting with underscore
obj._method()
obj._private_method()
...
# Accessing ANY attribute starting with underscore
obj._cache
obj._state
obj._config
obj._data
obj._internal_value
obj._private_attr
assert obj._something == value

# Name mangling bypass
obj._ClassName__private_method()
obj._ClassName__secret

# Dunder/internal access
Class.__ray_actor_class__
Class.__wrapped__
obj.__dict__["key"]
obj.__class__.__name__  # (when used for testing internals)

# ✅ GOOD - Test through public interface ONLY:
...
**EXHAUSTIVE LIST OF PYTHON PRIVATE PATTERNS TO DETECT:**

```
_method(), _attr, __method(), _Class__method
_setup, _teardown, _init, _cleanup, _reset, _clear
_pre_*, _post_*, _on_*, _before_*, _after_*
_execute, _run, _process, _handle, _dispatch
_validate, _check, _verify, _ensure, _assert
_fetch, _load, _save, _store, _cache, _retrieve
_parse, _format, _convert, _transform, _serialize
_build, _create, _make, _generate, _construct
_get_*, _set_*, _update_*, _delete_*, _remove_*
_internal_*, _private_*, _hidden_*, _impl_*
_do_*, _perform_*, _apply_*, _compute_*
__dict__, __class__, __module__, __wrapped__
__ray_actor_class__, __celery_*, __django_*
```
...
**The test is INCORRECT if it accesses ANYTHING that:**

1. **Starts with underscore** (`_method`, `_attr`, `__method`)
...
5. **Uses language-specific bypass methods** (`.send()`, `setAccessible()`, `instance_variable_get`)
6. **Is a framework-internal attribute** (`__ray_actor_class__`, `__wrapped__`, etc.)
7. **Is a lifecycle hook or internal callback** (`_pre_*`, `_post_*`, `_on_*`)
...
6. **Private access tests**: Tests that directly access private methods, functions, classes, or attributes instead of testing through the public interface. This includes using reflection, name mangling, `.send()` in Ruby, `(obj as any)` in TypeScript, or other language-specific tricks to bypass visibility
...
- **result**: "YES" if all code patch features have at least one test AND no unrelated tests exist AND no static tests exist AND no app config files in test patch AND no non-resilient imports AND no F2P issues AND no separation of concerns violations AND no private access tests. "NO" otherwise.
...
- **privateAccessTests**: Tests that directly access private, protected, or internal methods/functions/classes/attributes instead of testing through the public interface. This includes using reflection, name mangling, `.send()` in Ruby, or other language-specific tricks to bypass visibility. Empty array if all tests use the public API.
...
### Example 14: Private Access Tests - Python (INCORRECT)
...
{
  "result": "NO",
  "justification": "Tests directly access private methods and attributes instead of testing through the public interface (get_user method).",
  "missingTests": [],
  "unnecessaryTests": [],
  "staticTests": [],
  "appConfigInTestPatch": [],
  ...
  "separationOfConcerns": [],
  "privateAccessTests": [
    "test_fetch_from_db_directly directly calls private method _fetch_from_db instead of testing through get_user",
    "test_validate_user_id_with_name_mangling uses Python name mangling (_UserManager__validate_user_id) to access private method",
    "test_cache_is_populated directly accesses private attribute _cache instead of verifying caching behavior through public API"
  ]
}
```
</example_output>

**Note:** The correct approach would be to test the caching behavior through the public `get_user` method, not by directly accessing private members.
...
### Example 18: Correct Testing Through Public Interface (CORRECT)
...
{
  "result": "YES",
  "justification": "All tests verify the CacheManager behavior through its public interface (get and set methods). The eviction logic is tested by observing its effects through public methods, not by accessing private members.",
  "missingTests": [],
  "unnecessaryTests": [],
  "staticTests": [],
  "appConfigInTestPatch": [],
...
**Note:** This is the correct approach - testing the private eviction behavior by observing its effects through the public API, rather than directly calling `_evict_oldest` or accessing `_cache`.

### Example 19: Private Access Tests - Python Lifecycle Hooks (INCORRECT)
...
+@pytest.fixture
+def fastapi_node():
+    # BAD - Accessing internal Ray class attribute
...
+        # BAD - Directly calling private lifecycle hook
...
+            # BAD - Directly calling private _execute method
...
{
  "result": "NO",
  "justification": "Tests directly call private methods (_pre_loop_hook, _execute) and access internal class attributes (__ray_actor_class__) instead of testing through the public start() method.",
  "missingTests": [
    "Missing test for public start() method that verifies SSL files are created",
    "Missing test for public start() method that verifies uvicorn configuration"
  ],
  "unnecessaryTests": [],
  "staticTests": [],
  "appConfigInTestPatch": [],
  ...
  "separationOfConcerns": [],
  "privateAccessTests": [
    "Fixture fastapi_node accesses internal __ray_actor_class__ attribute instead of using proper test doubles",
    "test_pre_loop_hook_generates_certificate directly calls private method _pre_loop_hook() instead of testing via public start()",
    "test_pre_loop_hook_generates_private_key directly calls private method _pre_loop_hook() instead of testing via public start()",
    "test_execute_configures_uvicorn directly calls private method _execute() instead of testing via public start()"
  ]
}
```
</example_output>

**How tests SHOULD be written for this case:**

```python
# GOOD - Testing through public interface
class TestFastAPIServer:
    def test_start_generates_ssl_certificates(self, temp_dir):
        server = create_test_server()  # Use proper test factory
        
        server.start()  # Call PUBLIC method
        
        assert os.path.exists("certificate.pem")
        assert os.path.exists("privatekey.pem")
    
    def test_start_configures_uvicorn_with_ssl(self, temp_dir):
        ...
        with mock.patch("api_server.uvicorn") as mock_uvicorn:
            ...
            assert config_call.kwargs["ssl_keyfile"] == "privatekey.pem"
            assert config_call.kwargs["ssl_certfile"] == "certificate.pem"
```

**Key patterns detected in this example:**
...
- Flag tests that directly access private methods, functions, classes, or attributes instead of testing through the public interface
