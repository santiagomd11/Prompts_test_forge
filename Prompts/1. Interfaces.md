...
Use this decision tree for every code patch:

### For NEW FILES:

**Step 1: Does the new file contain callable public elements (classes, functions, methods)?**

- **YES** → Document BOTH:
  - The file itself
  - AND each public class, function, and method inside

- **NO** (file only has constants, types, re-exports) → Document ONLY:
  - The file itself (only if it is imported/used by other files in the codebase)
  - Do NOT document individual constants or types

### For MODIFIED FILES:

Only document NEW public elements added by the patch:
- New public classes
- New public functions
- New public methods
- **Renamed elements count as NEW only if their logic also changed** - if a class, function, or method is renamed AND its internal logic/behavior was modified, the new name is a new public interface. A pure rename with no logic change does NOT count as a new interface.

Do NOT document:
- The file itself (it already existed)
- Pre-existing elements that were not renamed
- Modified existing elements (unless renamed with logic changes)
- Pure renames with no logic change (just a name swap)

### Always EXCLUDE:

- Private/internal elements (language-specific rules apply)
- Constructors (`__init__`, `constructor`, `initialize`, etc.)
- Constants and variables
- Type definitions (interfaces, types, enums)
- Fields and properties
- ORM associations (`has_many`, `belongs_to`, `ForeignKey`, etc.)
- Nested functions
- Pre-existing interfaces

### Summary Table:

| Scenario | Document File? | Document Callables? |
|----------|---------------|---------------------|
| New file with classes/functions/methods | YES | YES |
| New file with only constants/types (if imported) | YES | NO |
| Modified file with new public elements | NO | YES (new ones only) |
| Modified file with only private changes | NO | NO (N/A) |
| Modified file with only associations/constants | NO | NO (N/A) |
...
### New Files That Export Public Elements

When a code patch creates a new file with public callable elements (classes, functions, methods), you must document BOTH:

1. **The file itself** - as it serves as the entry point that other code imports/requires
2. **All individual public callable elements** - each class, function, or method that is publicly accessible

This is NOT redundant. The file represents the importable unit, while the individual elements represent the callable interfaces within that file. Both levels are important for understanding the API surface area.

**Examples by language:**
- Python: A new `utils/helpers.py` with functions → document the file AND each function
- JavaScript/TypeScript: A new `services/auth.ts` with classes → document the file AND the class AND public methods
- Java: A new `UserService.java` with methods → document the class AND each public method
- Go: A new `pkg/validator/validator.go` with functions → document the package file AND each exported function
- Ruby: A new `lib/formatters/currency.rb` with a class → document the file AND the class AND public methods
- C++: A new `include/utils.hpp` with functions → document the header AND each public function
- C#: A new `Services/PaymentService.cs` with methods → document the class AND each public method

### Special Case: File is the ONLY Public Interface

This case ONLY applies when a new file contains NO callable public elements (no classes, functions, or methods).

In some cases, a new file is created but its contents do NOT qualify as individual public interfaces. This happens when the file contains ONLY non-callable elements:

**File contains ONLY:**
- Constants and variables (non-callable exports)
- Type definitions (TypeScript types, interfaces, enums)
- Re-exports from other files (barrel/index files)
...
In these cases, ONLY the file itself should be documented as a public interface (and only when it is imported/used by other parts of the codebase). The file is importable and serves as a public interface, but none of its individual contents meet the criteria for being documented as separate interfaces.

**IMPORTANT:** If a file contains ANY callable elements (classes, functions, methods), those MUST be documented IN ADDITION to the file itself. This special case does NOT apply to files with callable elements.
...
- **JavaScript/TypeScript**: `ClassName.methodName` or `file-name.ClassName.methodName` (PascalCase class, camelCase method). The module prefix is optional — `ReviewController.getReviews` and `review.controller.ReviewController.getReviews` are both acceptable. However, if a module prefix IS used, it must preserve the exact file name characters (dashes, dots, casing). For example, for `metrics-cache.ts`, writing `metrics-cache.ClassName` is correct but `metricsCache.ClassName` or `metricscache.ClassName` is INCORRECT because it alters the original file name.
...
- Type: file
- Input: None
- Output: AuthService class and authentication functions
- Description: Authentication service file providing user authentication and token management functionality.
...
```json
{
  "correctness_label": "CORRECT",
  "issues": [],
  "explanation": "The public interfaces correctly identify all public exports from the new auth.ts file: the auth.ts file itself, the AuthService class, its public login method, and the two exported standalone functions validateCredentials and refreshToken. The file is documented as it serves as the entry point for importing these authentication utilities. The private authenticateUser method is correctly excluded as it is marked private."
}
```
</example_output>
...
- Type: file
- Input: None
- Output: Validation functions and Validator class
- Description: Validation utilities file providing email and phone validation functions and a configurable Validator class.
...
```json
{
  "correctness_label": "CORRECT",
  "issues": [],
  "explanation": "The public interfaces correctly identify all public elements from the new validators.py file: the validators.py file itself, the validate_email and validate_phone functions, the Validator class, and its public validate method. The file is documented as it serves as the entry point for importing these validation utilities. The private _sanitize_input function and _check_field method are correctly excluded due to their underscore prefix indicating private visibility in Python."
}
```
</example_output>
...
- Description: Configuration file containing application settings constants including database connection, timeouts, and API settings.
```
</example_public_interfaces>
...
```json
{
  "correctness_label": "CORRECT",
  "issues": [],
  "explanation": "The public interfaces correctly document only the settings.py file itself. Although the file contains multiple constants that will be imported by other parts of the codebase, constants are non-callable exports and should not be documented as individual interfaces according to the rules. The file as a whole serves as the public interface."
}
```
</example_output>
...
- Description: Barrel file that re-exports component classes and type definitions from the components directory.
```
</example_public_interfaces>
...
```json
{
  "correctness_label": "CORRECT",
  "issues": [],
  "explanation": "The public interfaces correctly document only the index.ts file itself. The file only re-exports existing elements from other files and type definitions, which do not qualify as new individual public interfaces. The file serves as a convenient entry point for importing components and is itself the public interface."
}
```
</example_output>
...
```json
{
  ...
  "explanation": "The public interfaces incorrectly document individual constants. According to the rules, constants and variables are non-callable exports and should not be documented as individual interfaces. Only the constants.go file itself should be documented as a public interface since the file will be imported by other packages."
}
```
</example_output>
...
- Description: Type definition file containing interfaces and types for API data structures including User, Post, ApiResponse, and UserRole.
```
</example_public_interfaces>
...
```json
{
  "correctness_label": "CORRECT",
  "issues": [],
  "explanation": "The public interfaces correctly document only the api.ts file itself. The file contains TypeScript interfaces, type aliases, and enums, which are all type definitions that should not be documented as individual interfaces according to the rules. The file as a whole provides these type definitions to other parts of the codebase and is the public interface."
}
```
</example_output>
...
### Example 17: New Ruby File with Class and Methods - Document All Levels

<example_code_patch>
```ruby
diff --git a/app/workers/daily_sms_job.rb b/app/workers/daily_sms_job.rb
new file mode 100644
+class DailySmsJob
+  include Sidekiq::Worker
+
+  def perform(*args)
+    users = User.where(sms_enabled: true)
+    users.each do |user|
+      send_sms(user)
+    end
+  end
+
+  def send_sms(user)
+    SmsService.deliver(user.phone, daily_message(user))
+  end
+
+  private
+
+  def daily_message(user)
+    "Hello #{user.name}, here is your daily update!"
+  end
+end
```
</example_code_patch>
...
<example_public_interfaces>
```
- Path: /app/app/workers/daily_sms_job.rb
- Name: daily_sms_job.rb
- Type: file
- Input: None
- Output: DailySmsJob class
- Description: Sidekiq worker file for daily SMS notification jobs.

- Path: /app/app/workers/daily_sms_job.rb
- Name: DailySmsJob
- Type: class
- Input: None
- Output: DailySmsJob instance
- Description: Sidekiq worker class that processes daily SMS notifications to enabled users.

- Path: /app/app/workers/daily_sms_job.rb
- Name: DailySmsJob.perform
- Type: method
- Input: *args: Array
- Output: void
- Description: Executes the job to send daily SMS notifications to all users with SMS enabled.

- Path: /app/app/workers/daily_sms_job.rb
- Name: DailySmsJob.send_sms
- Type: method
- Input: user: User
- Output: void
- Description: Sends an SMS message to the specified user.
```
</example_public_interfaces>
...
```json
{
  "correctness_label": "CORRECT",
  "issues": [],
  "explanation": "The public interfaces correctly document all levels for this new file: the daily_sms_job.rb file, the DailySmsJob class, and its two public methods (perform and send_sms). This is correct because the file contains callable public elements, so both the file AND all individual public elements must be documented. The private daily_message method is correctly excluded as it appears after the private keyword."
}
```
</example_output>

### Example 18: Renamed with Logic Changes Count as NEW Public Interfaces
...
+        channel = self.resolve_channel(user)
+        channel.deliver(user, message)
+        self.log_dispatch(user, message)

-    def format_message(self, message):
-        return message.strip()
+    def prepare_message(self, message):
+        sanitized = message.strip()
+        return self.apply_template(sanitized)
```
</example_code_patch>

<example_public_interfaces>
```
- Path: /app/app/services/notification_service.py
- Name: notification_service.NotificationService
- Type: class
- Input: None
- Output: NotificationService instance
- Description: Service class for handling notifications through multiple channels.

- Path: /app/app/services/notification_service.py
- Name: notification_service.NotificationService.dispatch
- Type: method
- Input: user: User, message: str
- Output: None
- Description: Dispatches a notification to the specified user through the appropriate channel.

- Path: /app/app/services/notification_service.py
- Name: notification_service.NotificationService.prepare_message
- Type: method
- Input: message: str
- Output: str
- Description: Sanitizes and applies a template to the message for sending.
```
</example_public_interfaces>
...
```json
{
  "correctness_label": "CORRECT",
  "issues": [],
  "explanation": "The public interfaces correctly identify the renamed elements as new public interfaces. The class was renamed from EmailNotifier to NotificationService and the methods were renamed from send_notification to dispatch and format_message to prepare_message. These count as new interfaces because the logic also changed: dispatch now resolves channels and logs instead of just emailing, and prepare_message now applies templates instead of just stripping whitespace. Renames with logic changes represent new public interfaces."
}
```
</example_output>

### Example 19: Pure Rename Without Logic Change - NOT a New Interface
...
Reasoning: The code patch only renames the class (MathHelper → Calculator) and methods (compute_sum → add, compute_difference → subtract) without changing their internal logic. Pure renames with no logic changes do not count as new public interfaces.
```
</example_public_interfaces>
...
```json
{
  "correctness_label": "CORRECT",
  "issues": [],
  "explanation": "The public interfaces correctly indicate N/A. Although the class was renamed from MathHelper to Calculator and the methods from compute_sum/compute_difference to add/subtract, the internal logic remains identical (return a + b and return a - b). Pure renames without logic changes do not count as new public interfaces."
}
```
</example_output>
...
- When new files are created with callable public elements (classes, functions, methods), document BOTH the file AND all individual public elements
- Files are NOT redundant when they contain callable elements - both the file and its contents should be documented
- Only document the file alone when it contains ONLY non-callable exports (constants, types, re-exports) and is imported by other code
