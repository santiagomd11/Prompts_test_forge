...
- **JavaScript/TypeScript**: `file-name.ClassName.methodName` (use exact file name without extension as module name, preserving dashes, dots, and original casing; PascalCase class, camelCase method). For example, for a file `metrics-cache.ts`, use `metrics-cache.ClassName.methodName`, NOT `metricsCache.ClassName.methodName`.
