...
- **result**: "YES" if all code patch features have at least one test AND no unrelated tests exist AND no static tests exist AND no app config files in test patch AND no non-resilient imports. "NO" otherwise.
...
- **staticTests**: Tests that only verify existence of functions/methods/classes but do not actually test their behavior. Empty array if all tests actually exercise functionality.
